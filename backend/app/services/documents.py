...
class DocumentsService:
    ...
    def _base_template_context(self, tenant_id: int) -> dict[str, Any]:
        ...
        logo_url = self._resolve_logo_src(settings.document_logo_url if settings and hasattr(settings, "document_logo_url") else None)
        return {
            "tenant": {
    ...
    def _resolve_logo_src(self, logo_url: str | None) -> str | None:
        if not logo_url:
            return None
        ...
        if relative_path.startswith("uploads/"):
            ...
            file_path = uploads_dir / Path(relative_path).name
